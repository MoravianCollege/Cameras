...
@patch.object(manager, "update_processing_screen", side_effect=manager.update_processing_screen)
def test_general_run_through(mock_run_camera, mock_run_running, mock_run_results, mock_camera_update, mock_running_update, mock_results_update, mock_processing, mock_processing_update):
    ...
    mock_processing_update.assert_called
    assert manager.current_screen == 4
...
@patch.object(manager, "run_results_screen")
@patch.object(manager, "do_processing")
def test_advance_method(mock_run_camera, mock_run_running, mock_run_results, mock_do_processing):
    ...
    mock_do_processing.assert_called
